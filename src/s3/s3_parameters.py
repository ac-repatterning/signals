...
class S3Parameters:
    ...
    def __build_collection(self, dictionary: dict) -> s3p.S3Parameters:
        ...
        # Parsing variables
        region_name = self.__secret.exc(secret_id=self.__configurations.project_key_name, node='region')
        internal = self.__secret.exc(secret_id=self.__configurations.project_key_name, node='internal')
        external = self.__secret.exc(secret_id=self.__configurations.project_key_name, node='external')
        configurations = self.__secret.exc(secret_id=self.__configurations.project_key_name, node='configurations')
